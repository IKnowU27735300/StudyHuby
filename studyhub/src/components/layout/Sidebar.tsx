...
import { BookOpen, FileText, LayoutDashboard, GraduationCap, Settings, LogOut, ChevronRight, Share2, Flame, Award, Loader2 } from 'lucide-react';
...
export default function Sidebar() {
  ...
  return (
    <aside className={`fixed left-0 top-0 hidden h-screen ${isCollapsed ? 'w-20' : 'w-72'} flex-col gap-6 border-r border-white/10 bg-slate-950/50 p-4 backdrop-blur-xl lg:flex z-40 transition-all duration-300 overflow-hidden`}>
      <Link href="/dashboard" className={`flex items-center hover:opacity-80 transition-opacity ${isCollapsed ? 'justify-center' : 'gap-3 px-2'}`}>
        ...
        {!isCollapsed && <h1 className="text-2xl font-bold tracking-tight text-white font-outfit whitespace-nowrap">StudyHub</h1>}
      </Link>
      ...
      {!isCollapsed && (
        <div className="rounded-2xl bg-gradient-to-br from-primary/20 to-purple-500/20 p-4 border border-white/5 relative overflow-hidden group">
      ...
      <div className="border-t border-white/5 pt-4 flex flex-col gap-2">
        <Link 
          ...
          className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3'} rounded-xl px-4 py-3 text-muted-foreground transition-all hover:bg-white/10 hover:text-white`}
        >
